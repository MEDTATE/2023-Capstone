...
// looks more similar to other techniques with these settings
...
float4 FXAACopyAndLumaPS( float4 screenPos : SV_Position ) : SV_Target
{
 ...
 #else
    color.a = dot(colorGamma.rgb, float3(0.299, 0.587, 0.114)); // compute luma
 #endif
...
float4 FXAAEffectPS( float4 screenPos : SV_Position ) : SV_Target
{
    ...
    return FxaaPixelShader( screenPos.xy * g_FXAA.rcpFrame.xy, float4( 0, 0, 0, 0 ), tex, g_FXAA.rcpFrame, g_FXAA.rcpFrameOpt );
}
