...
Texture2D           g_txZoomSrcColor    : register( t0 );
Texture2D<float>    g_txZoomSrcEdges    : register( t1 );
...
float4 PS_Zoom( const float4 screenPos : SV_Position ) : SV_Target
{
    ...
    return float4( g_txZoomSrcColor.Load( int3( texCoord, 0 ) ).rgb, 1 );
}

#define CMAA_INCLUDE_JUST_DEBUGGING_STUFF 1
#include "CMAA.hlsl"

float4 PS_CMAAZoom( const float4 screenPos : SV_Position ) : SV_Target
{
    ...
    float2 texCoord = RectToRect( screenPos.xy, destRectCenter, destRectSize, srcRectCenter, srcRectSize );
    float3 colour = g_txZoomSrcColor.Load( int3( texCoord, 0 ) ).rgb;

    if( g_ZoomShowEdges )
    {
        float4 thisPixRect, thisPixRectBig4;
        thisPixRect.xy = RectToRect( screenPos.xy, destRectCenter, destRectSize, srcRectCenter, srcRectSize );
        thisPixRect.zw = RectToRect( screenPos.xy, destRectCenter, destRectSize, srcRectCenter, srcRectSize );
        thisPixRect.xy = RectToRect( floor( thisPixRect.xy ), srcRectCenter, srcRectSize, destRectCenter, destRectSize );
        thisPixRect.zw = RectToRect( ceil(  thisPixRect.zw ), srcRectCenter, srcRectSize, destRectCenter, destRectSize );
        ...
        if( dist < 1.1 ) //&& chessPattern )
        {
            ...
            UnpackBlurAAInfo( g_txZoomSrcEdges.Load( int3( texCoord.xy, 0 ) ).r, packedEdges, shapeType );
